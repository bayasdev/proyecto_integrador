...
export class CatalogService {
  ...
  upload_items(catalog: string, items: any[]): Promise<any> {
    ...
    return this.http.post(environment.api_catalog + catalog + '/upload_items', data, this.options).toPromise();
  }

  get_items(catalog: string, output_model: any): Promise<any> {
    const data = { output_model: output_model };
    return this.http.post(environment.api_catalog + catalog + '/get_items', data, this.options).toPromise();
  }

  get_item(catalog: string, item_id: string, output_model: any): Promise<any> {
    const data = { item_id: item_id, output_model: output_model };
    return this.http.post(environment.api_catalog + catalog + '/get_item', data, this.options).toPromise();
  }

  update_item(catalog: string, item_id: string, item: any): Promise<any> {
    const data = { item_id: item_id, item: item };
    return this.http.post(environment.api_catalog + catalog + '/update_item', data, this.options).toPromise();
  }

  delete_item(catalog: string, item_id: string): Promise<any> {
    const data = { item_id: item_id };
    return this.http.post(environment.api_catalog + catalog + '/delete_item', data, this.options).toPromise();
  }
  ...
  search_items(catalog: string, attribute: string, value: any, output_model: any): Promise<any> {
    ...
    return this.http.post(environment.api_catalog + catalog + '/search_items', data, this.options).toPromise();
  }
}
